...
class AssetNNetImage {
  AssetNNetImage({this.imageUrl, this.asset});
...
class ContentEditPage extends StatefulWidget {
  const ContentEditPage({Key? key, required this.contentNo}) : super(key: key);
  final int contentNo;
...
class _ContentEditPageState extends State<ContentEditPage> {
  ...
  AssetNNetImage? coverPic;
  List<AssetNNetImage> selectPics = [];
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      loading = false;
      List<String> photos = data!.images!.split(',');
      ...
      if(data != null) {
        setState((){
          _tecTitle.text = data!.title!;
          content = data!.message!;
          initTags = data!.keyword!.split(',');
        });
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: loading?Center(child: CircularProgressIndicator()):_buildBody(context),
    );
  }

  Widget _buildBody(BuildContext context) {

    print("initTags : " + initTags.toString());
    return SafeArea(
    ...
    if(selectPics.isEmpty) {
      ...
      return GridView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            AssetNNetImage item = selectPics[index];
            return Stack(
              children: [
                item.imageUrl!=null?Image.network(item.imageUrl!, width: cellWidth, height: cellWidth, fit: BoxFit.cover):FutureBuilder<dynamic> (
                    future: item.asset!.thumbnailData, // a previously-obtained Future<String> or null
  ...
  Widget _imagePopup({required AssetNNetImage item, required double cellWidth}) => PopupMenuButton<int>(
    padding: EdgeInsets.zero,
    icon: SizedBox(
      width: cellWidth,
      height: cellWidth,
  ...
  Future<void> onAddClick(BuildContext context) async {
    ...
    List<AssetEntity> selectPics_ = [];
    for(AssetNNetImage item in selectPics) {
      if(item.asset != null) {
        selectPics_.add(item.asset!);
      }
    }
    final List<AssetEntity>? result = await AssetPicker.pickAssets(
      context,
      pickerConfig: AssetPickerConfig(
        selectedAssets: selectPics_,
        maxAssets: 12,
    ...
    if(result != null) {
      selectPics.removeWhere((element) => element.asset != null);
      setState(() {

        for(var asset in result) {
          selectPics.add(
            AssetNNetImage(asset: asset)
          );
        }
  ...
  Future<void> saveContent(BuildContext context) async {
    ...
    // 파일을 업로드한다.
    for(AssetNNetImage assetItem in selectPics) {
      if(assetItem.asset == null) {
        images.add(assetItem.imageUrl!);
        continue;
      }

      AssetEntity asset = assetItem.asset!;
      ...
      if(uploadFile != null) {
        ...
        if(res.statusCode == 200) {
          ...
          if(coverPic!.asset != null) {
            if(asset == coverPic!.asset!) {
    ...
    var body = {
      'contentNo': widget.contentNo.toString(),
      'category': category,
    ...
    var postRes = await Api().put(Uri.parse(url), body: body);
