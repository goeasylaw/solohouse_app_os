...
class SolohouseRepository with ChangeNotifier {
  static final SolohouseRepository _singleton = SolohouseRepository._internal();

  factory SolohouseRepository() { return _singleton; }

  SolohouseRepository._internal();

  List<SolohouseModel> list = [];
  static String category = '솔로하우스';
  String startDate = '';
  bool nextPage = false;
  ...
  Future<SolohouseModel?> getOneReload(int no) async {
    SolohouseModel? seleItem;
    for(var item in list) {
      if(item.contentNo == no) {
        seleItem = item;
      }
    ...
    if(response.statusCode == 200) {
      SolohouseListModel data = SolohouseListModel.fromJson(jsonDecode(response.body));
      if(seleItem == null) {
        list.add(data.contentList!.first);
      } else {
        int index = list.indexOf(seleItem);
        list.insert(index, data.contentList!.first);
        list.remove(seleItem);
      }
      return data.contentList!.first;
    } else {
      SmartDialog.showToast('Server Error');
